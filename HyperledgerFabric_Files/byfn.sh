...
function networkUp () {
  ...
  if [ "${IF_COUCHDB}" == "couchdb" ]; then
      CHANNEL_NAME=$CHANNEL_NAME TIMEOUT=$CLI_TIMEOUT DELAY=$CLI_DELAY docker stack deploy -c  docker-stack.yml test_hl
  else
      CHANNEL_NAME=$CHANNEL_NAME TIMEOUT=$CLI_TIMEOUT DELAY=$CLI_DELAY docker stack deploy -c  docker-stack.yml test_hl
  fi
  if [ $? -ne 0 ]; then
...
function replacePrivateKey () {
  ...
  cp docker-stack-template.yml docker-stack.yml
  ...
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-stack.yml #replacing CA1_PRIVATE_KEY with private key
  cd crypto-config/peerOrganizations/org2.example.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA2_PRIVATE_KEY/${PRIV_KEY}/g" docker-stack.yml
  # If MacOSX, remove the temporary backup of the docker-compose file
  if [ "$ARCH" == "Darwin" ]; then
    rm docker-stack.ymlt
  fi
}
...
COMPOSE_FILE=docker-stack-template.yml
#
